...
| **AI Fashion Billboard, BYO-key** (Settings → AI Provider → Image generation) — OpenAI images, Gemini image, a local Stable Diffusion (Automatic1111) backend, plus the original proxy contract, unchanged. **"Generate Pattern Pieces From This"** on the generated photo drafts real editable pieces onto the canvas (same pipeline as the AI Pattern Generator's own image upload) — not just the existing "Use as Background Trace" manual path | ✅ Working — proxy option is byte-for-byte compatible with existing `server/billboard-proxy/worker.js` deployments |
